...
module.exports = function(logger, poolConfig){
    ...
    var coinName = poolConfig.coinName;
    ...
    this.handleAuth = function(workerName, password, authCallback){
        ...
        connection.query(
            'SELECT password FROM `bP_main`.`pool_worker` WHERE username = LOWER(?)',
            ...
            function(err, result){
                if (err){
                    ...
                    if(mposConfig.autoCreateWorker){
                        ...
                        connection.query(
                            'SELECT id,username FROM `bP_main`.`accounts` WHERE username = LOWER(?)',
                            [account.toLowerCase()],
                            function(err, result){
                                if (err){
                                    ...
                                    connection.query(
                                        "INSERT INTO `bP_main`.`pool_worker` (`account_id`, `username`, `password`, `current_coin`) VALUES (?, ?, ?, ?);",
                                        [result[0].id,workerName.toLowerCase(),password,coinName],
                                        function(err, result){
                                            if (err){
                    ...
                    connection.query(
                        "UPDATE `bP_main`.`pool_worker` set `current_coin` = ? WHERE `username` = LOWER(?)",
                        [coinName, workerName.toLowerCase()],
                        function(err, result){
                            if (err){
                                logger.error(logIdentify, logComponent, 'Database error when setting current coin: ' +
                                    JSON.stringify(err));
                                authCallback(false);
                            }else {
                                authCallback(true);
    ...
    this.handleDifficultyUpdate = function(workerName, diff){
        ...
        connection.query(
            'UPDATE `bP_main`.`pool_worker` SET `difficulty` = ' + diff + ' WHERE `username` = ' + connection.escape(workerName),
            function(err, result){
                if (err)
